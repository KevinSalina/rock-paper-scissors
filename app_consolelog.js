...
// Prompts for user Hand
function userHand(){
    let userInput = prompt(`Select a hand! (Rock, Paper, Scissors) - Round: ${round}` ).toLowerCase().charAt(0)
        while(!userInput || !gameHands.includes(userInput)){
            userInput = prompt(`You must make a valid selection! Rock, Paper, Scissors - Round: ${round}`).toLowerCase().charAt(0)
        }
    return userInput
}
